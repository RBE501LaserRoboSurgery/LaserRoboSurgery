...
void markerListenerCB(
    ...
    g_R = g_quat.matrix(); */
    /////////////////////////////   Change these to read x,y,z,Rx,Ry,Rz from sensor/msg   ///////////////// 
...
vector<Vectorq6x1> gen_trajectory_straight( Eigen::Affine3d final) {
    float d=dist(final,g_q_state);
    density=max(int(dist(final,g_q_state))/2,1);s
    vector<Vectorq6x1> lst;
    Eigen::Affine3d temp=final;
    std::vector<Vectorq6x1> q6dof_solns;
    Eigen::Vector3d cpy;
    Vectorq6x1 prev_jt_st=home_state;
    for (int i = 0; i < density; i++) {
        nxt_pt=g_q_state+(i+1)/density*d
        cpy[0]=nxt_pt[0];cpy[1]=nxt_pt[1];cpy[2]=nxt_pt[2];
        temp.translation()=cpy;

        ik_solver.ik_solve(A_flange_des_DH);
        ik_solver.get_solns(q6dof_solns);
        Vectorq6x1 j=get_nearest(q6dof_solns,prev_jt_st);
        lst.push_back(j);
        prev_jt_st= j;
         } 
    return lst;
...
float dist(Eigen::Affine3d p1,Vectorq6x1 p2)
    p1=p1.translation();
    return math.sqrt(pow(p1[0]*p1[0]-p2[0]*p2[0]],2)+pow(p1[1]*p1[1]-p2[1]*p2[1]],2)+pow(p1[2]*p1[2]-p2[2]*p2[2]],2));
...
void stuff_shaped_epsilon( std::vector<Vectorq6x1> all_qvec, trajectory_msgs::JointTrajectory &new_trajectory) {
    
    ///////////// Needs fixing and linking to Jacobian
...
Vectorq6x1 get_nearest(std::vector<Vectorq6x1> all_qvec, Vectorq6x1 prev) {
    float dt=0;
    int ri=0;
...
float dthetas(Vectorq6x1 p1,Vectorq6x1 p2){
    return sqrt(pow(p1[0]*p1[0]-p2[0]*p2[0]],2)+pow(p1[1]*p1[1]-p2[1]*p2[1]],2)+pow(p1[2]*p1[2]-p2[2]*p2[2]],2));
}
...
int main(int argc, char** argv) {
    ...
    int nsolns;
    enum traj_type { test,straight, circle };
    traj_type r=test;
    ...
    while(ros::ok()) {
            ...
            if (g_trigger) {
                ...
                case straight:
                    ...
                    // qvec = q6dof_solns[soln]; // arbitrarily choose first soln                    
                        // stuff_trajectory(qvec,new_trajectory); 
                    stuff_shaped_trajectory(gen_trajectory_straight(A_flange_des_DH), new_trajectory);
                            pub.publish(new_trajectory);
